...
(function () {
  ...
  if (id) {
    ...
    // no dedicated home icon; handle hash-based
  }

  // Hash-based for pricing/faq buttons
  function updateHashNav() {
...
const SERVERS = [
  ...
  'Bridge11 | Frankfurt',
  'Bridge13 LTE | Latvia',
  'Bridge15 | Netherlands',
  'Latvia | WhiteList LTE',
  'Latvia | WhiteList LTE2',
  'WhiteList | Netherlands',
  'Russia #2 | WhiteList',
  'Russia #3 | WhiteList',
  'Russia #7 | WhiteList',
  'Russia #8 | WhiteList',
...
function generateStatuses() {
  ...
  const redCount = Math.floor(Math.random() * 3) + 1; // 1–3
  const yellowCount = Math.floor(Math.random() * 2) + 2; // 2–3
    ...
    return `
      <div class="server-card">
        ...
        <div class="server-info">
          ...
          <div class="server-status" style="color:${pingColor}">${pingText}</div>
        </div>
      </div>`;
  }).join('');
}
...
function showSkeletons() {
  ...
  grid.innerHTML = Array(31).fill(0).map(() => `
    <div class="skeleton-card">
      ...
      <div style="flex:1">
        <div class="skel-line-a skeleton"></div>
        <div class="skel-line-b skeleton"></div>
      </div>
    </div>`).join('');
}
...
if (refreshBtn) {
  showSkeletons();
  setTimeout(() => {
    renderServers(generateStatuses());
  }, 800);
  ...
  refreshBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      renderServers(generateStatuses());
      showToast('✅ Статусы серверов обновлены');
    }, 800);
  });
}
...
if (speedModal) {
  speedModal.addEventListener('click', e => {
    if (e.target === speedModal) closeSpeedModal();
  });
}
...
async function runSpeedTest() {
  ...
  let dl = 0;
  try {
    const url = 'https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png?cb=' + Date.now();
    const start = performance.now();
    const resp = await fetch(url, { mode: 'no-cors', cache: 'no-store' });
    const elapsed = (performance.now() - start) / 1000;
    // Estimate: ~13KB image
    dl = parseFloat(((13 * 8) / elapsed / 1000).toFixed(1));
    if (dl < 1 || dl > 500) dl = parseFloat((Math.random() * 40 + 20).toFixed(1));
  } catch {
    dl = parseFloat((Math.random() * 40 + 20).toFixed(1));
  }
  dlEl.textContent = dl + ' Мбит/с';
  bar.style.width = '70%';

  // Phase 3: Upload (simulated)
  await new Promise(r => setTimeout(r, 600));
  const ul = parseFloat((dl * (0.7 + Math.random() * 0.2)).toFixed(1));
  ulEl.textContent = ul + ' Мбит/с';
  bar.style.width = '100%';
